...
def generate(league):
    ...
    # Get valid neighboring schedules
    for bye, games in enumerate(league):
        for i in range(len(games)):
            for j in range(i + 1, len(games)):
                neighbor = swap_players(league, bye, i, j)
                if neighbor and is_valid_neighbor(neighbor):
                    neighbors.append(neighbor)

    return neighbors

def swap_players(league, bye, i, j):
    games = league.copy()
    game1, game2 = games[bye][i].copy(), games[bye][j].copy()
    for k in range(len(game1)):
        for l in range(len(game2)):
            ...
            games[bye][i], games[bye][j] = newgame_1, newgame_2
            return games

def is_valid_neighbor(neighbor):
    pairs = calculate_player_counts(neighbor)
    return check_validity(pairs)
